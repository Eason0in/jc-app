...
module.exports = async (e, data) => {
  try {
    ...
    const { filePath, range, isNeedTidy, acTidyObj } = data;
    ...
    const handleBars = ({ s, l }) => {
      ...
      for (let i = s; i < s + l; i++) {
        ...
        row.eachCell({ includeEmpty: false }, function (cell, colNumber) {
          ...
          let tNo = resultOrValue
            ? typeof resultOrValue === "string" && resultOrValue.toUpperCase()
            : undefined;
          ...
          const lenB =
            row.getCell(colNumber + 1).result ||
            row.getCell(colNumber + 1).value;
          const condS = lenB && typeof lenB === "number";

          const count =
            row.getCell(colNumber + 2).result ||
            row.getCell(colNumber + 2).value;
    ...
    const handleOthers = (linesArr) => {
      ...
      twentySevenRow.eachCell(
        { includeEmpty: false },
        function (cell, colNumber) {
          const resultOrValue = cell.result || cell.value; // 代號可能是直接英文或組成的
          const tNo = resultOrValue
            ? typeof resultOrValue === "string" && resultOrValue.toUpperCase()
            : undefined;
          const condF = tNo && numMap.has(tNo);
          ...
          if (tNo === "ID") {
            second =
              twentySevenRow.getCell(colNumber + 1).result ||
              twentySevenRow.getCell(colNumber + 1).value;
          ...
          const count =
            twentySevenRow.getCell(colNumber + 2).result ||
            twentySevenRow.getCell(colNumber + 2).value;
          const regex = new RegExp("x", "gi");
          const condT = count && regex.test(count);
      ...
      twentyNightRow.eachCell(
        ...
        function (cell, colNumber) {
          ...
          const second =
            twentyNightRow.getCell(colNumber + 1).result ||
            twentyNightRow.getCell(colNumber + 1).value;
          const secArr =
            second &&
            second
              .toString()
      ...
      thirtyThreeRow.eachCell(
        ...
        function (cell, colNumber) {
          ...
          const lenB =
            thirtyThreeRow.getCell(colNumber + 1).result ||
            thirtyThreeRow.getCell(colNumber + 1).value;
          const condS = lenB && typeof lenB === "number";

          const count =
            thirtyThreeRow.getCell(colNumber + 2).result ||
            thirtyThreeRow.getCell(colNumber + 2).value;
          const regex = new RegExp("x", "gi");
          const condT = count && regex.test(count);
    ...
    const handleTidy = () => {
      Object.entries(needTidyObj).forEach(([key, arr]) => {
        ...
        const arrangeObj = Object.values(arr)
          ...
          .sort(
            (pre, next) =>
              +next.num.replace("#", "") - +pre.num.replace("#", "")
          )
          .reduce((obj, item) => {
            const { num } = item;
            return {
        ...
        const tidiedArr = Object.values(arrangeObj).map((subArr) => {
          const tidiedRange =
            subArr[0].tNo === "AC"
              ? acTidyObj[subArr[0].num].selectBeamRange
              : range;

          let currentMax = Math.ceil(subArr[0].lenB / 10) * 10;
          let nextMax = currentMax - tidiedRange;

          for (let i = 0; i < subArr.length; i++) {
            while (subArr[i].lenB <= currentMax) {
              // CC 跳過不歸整 or [ac] or ac 該號數沒打勾
              if (
                subArr[i].tNo === "CC" ||
                subArr[i].isNotTidy ||
                (subArr[i].tNo === "AC" && !acTidyObj[subArr[i].num].isNeedTidy)
              ) {
    ...
    const handleTidyWriteBySheet = async () => {
      ...
      const obj = {};
      const orderBySheetName = tidyBySheetNameArr.sort((pre, next) =>
        pre.sheetName > next.sheetName ? 1 : -1
      );
